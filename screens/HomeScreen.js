...
const SAP_CHECK_STATUS_API = 'https://magicqa.tatahousing.com/Magicxpi4.13/MgWebRequester.dll?appname=IFSEMS_To_SAP&prgname=HTTP&arguments=-AHTTP_1%23Check_Status';

// Module-level cache so all expense cards share results — same DocumentNumber
// won't be refetched when the user switches tabs or re-renders.
...
/** Segmented-control tabs (scrollable, pills auto-size to content) */
          ...
          {tabs.map((label) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.85}
                style={styles.segmentTap} // note: no flex:1 here
              >
                {active ? (
                  <LinearGradient
                  ...
                  >
                    <Text style={[styles.segmentTextActive, { fontSize: fs }]}>
                      {label}
                    </Text>
                  </LinearGradient>
                ) : (
                  ...
                  >
                    <Text style={[styles.segmentText, { fontSize: fs }]}>
                      {label}
                    </Text>
                  </View>
                )}
...
const ExpenseCard = ({ item, onView, onEdit, onDelete }) => {
  ...
  const showEdit = isDraft; // hide edit for pending per requirement
  // const showDelete = isDraft || isPending;
  ...
  return (
    <View style={styles.card}>
      <View style={{ flex: 1 }}>
        ...
        { item.ExpenseData[0].SelfApprove === true? null :<Text style={styles.cardSub}>Approver : {item.ApproverEmail || 'N/A'}</Text>}
        <Text style={styles.cardSub}>Document No: {item.ExpenseData[0].ItemData.DocumentNo}</Text>
      ...
      <View style={{ alignItems: 'flex-end', justifyContent: 'space-between' }}>
        <Text style={styles.cardAmount}>{formatRs(item.TotalAmount || formatRs(item.ExpenseData[0].ItemData.ClaimAmount) || 0)}</Text>
       {item.ExpenseData[0].SelfApprove === true && item.ApprovalStatus === 'Approved'? <Text style={[styles.statusText, { color: badgeColor }]}>Self Approved</Text> :
...
export default function HomeScreen() {
  ...
  // Fetch and cache employee info from API
  const fetchAndCacheEmployeeInfo = async (email) => {
    try {
      ...
      await AsyncStorage.setItem('employee_info', JSON.stringify(data));

      console.log('Employee info cached successfully');
      return data;
    } catch (e) {
  ...
  // Fetch user email from AsyncStorage on mount
  useEffect(() => {
  ...
  // Fetch employee info when userEmail is available
  useEffect(() => {
    const loadEmployeeInfo = async () => {
      if (!userEmail) return;

      try {
        // First, try to load from AsyncStorage (cached data)
        const cachedData = await AsyncStorage.getItem('employee_info');
        if (cachedData) {
          try {
            const parsed = JSON.parse(cachedData);
            setEmployeeInfo(parsed);
            console.log('Employee info loaded from cache');
          } catch (parseErr) {
            console.warn('Error parsing cached employee info:', parseErr);
          }
        }

        // Then fetch fresh data from API
        await fetchAndCacheEmployeeInfo(userEmail);
      } catch (err) {
        console.error('Error loading employee info:', err);
        // If API call fails, we still have cached data if available
      }
    };
  ...
  const fetchExpenses = useCallback(async () => {
    if (!userEmail) {
      // Don't fetch if user email is not available
      return;
    }
    try {
      setLoading(true);
  ...
  const handleDelete = (item) => {
    ...
    if (statusKey === 'draft' || statusKey === 'pending') {
      Alert.alert(
        'Delete Expense',
        `Are you sure you want to delete "${item.ExpenseTitle || 'this expense'}"?`,
        [
